...
def calcula_custos(cidade_atual, cidades_para_visitar, matriz):
    relacao_distancias = {}
...
def obtem_feromonios(cidade_atual, cidades_para_visitar, matriz_feromonios):
    relacao_feromonios = {}
    for cidade_destino in cidades_para_visitar:
...
def calcula_probabilidades(feromonios_totais, custos_totais, alpha, betta):
    n = 1 / custos_totais
...
def percorrer_caminho(caminho, custo, matriz_feromonios):
    matriz_feromonios
...
def atualiza_melhor_caminho(caminho, custo, melhor_caminho):
...
def main(populacao, iteracoes):
    ...
    iteracoes = iteracoes #int(input('Numero de iterações (padrão 10): ') or "10")
    alpha = 1 #int(input('Valor do Alpha(padrão 1): ') or "1")
    betta = 1 #int(input('Valor do Betta(padrão 1): ') or "1")
    numero_formigas = populacao #int(input('Numero de Formigas (padrão 5): ') or "5")
    ...
    for iteracao in range(iteracoes):
        ...
        for formiga in range(numero_formigas):
            ...
            while cidades_visitar:
                distancias_totais = calcula_custos(cidade_atual, cidades_visitar, matriz)
                feromonios_totais = obtem_feromonios(cidade_atual, cidades_visitar, matriz_feromonios)
            ...
            atualiza_melhor_caminho(caminho, custo, melhor_caminho)
            custos.append(custo)

        
        for caminho, custo in zip(caminhos_por_formiga, custos_por_formiga):
            #print(f"caminho {caminho} e custo {custo}")
            percorrer_caminho(caminho, custo, matriz_feromonios)
